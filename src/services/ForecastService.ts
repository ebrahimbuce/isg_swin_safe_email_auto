...
import { HTMLGeneratorService } from './HTMLGeneratorService.js';
import type { AlertStatus } from './dto/HTMLGeneratorDTO.js';
import { SummaryFormatter } from '../utils/SummaryFormatter.js';
import type { ForecastResult, ForecastConfig } from './dto/ForecastDTO.js';
...
export type { ForecastResult, ForecastConfig };
...
export class ForecastService {
  ...
  async getForecast(): Promise<ForecastResult> {
    ...
    try {
      ...
      const [alertStatus] = await Promise.all([
        this.htmlGenerator.updateHTML({ detection: colorDetection }),
      ...
      const outputImagePath = await this.htmlGenerator.exportToImage({
        width: 1200, // Ancho final HD (ligeramente reducido)
        height: 2500, // Alto final HD (más alto para incluir footer, se ajusta proporcionalmente)
        format: 'png',
      });
  ...
  private async getImage(): Promise<Buffer> {
    return await this.imageProcessor.getImage({ url: this.url });
  }
  ...
  private async processImage(image: Buffer): Promise<Buffer> {
    return await this.imageProcessor.cropImage({
      image,
  ...
  private async saveImage(image: Buffer): Promise<void> {
    await this.imageProcessor.saveImage({ image, path: this.outputPath });
  }
  ...
  private async detectColorsMinimal(image: Buffer): Promise<ColorDetectionResult> {
    ...
    return await this.imageProcessor.detectColors({ image, threshold: 0.01 });
  }
