...
async function generateAllAlertImages() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      await imageProcessor.saveImage({ image: testImage, path: forecastPath });
      ...
      await htmlGenerator.exportToImage({ htmlPath: outputPath, width: 1100, height: 2500, format: 'png' });
