...
async function testColorDetection() {
    ...
    try {
        ...
        const image = await imageProcessor.getImage(testUrl);
        ...
        console.log(`   - Píxeles: ${Math.round((detection.redPercentage / 100) * detection.totalPixels).toLocaleString()}`);

        console.log('\n🟡 AMARILLO:');
        console.log(`   - Detectado: ${detection.hasYellow ? '✅ SÍ' : '❌ NO'}`);
        console.log(`   - Porcentaje: ${detection.yellowPercentage}%`);
        console.log(`   - Píxeles: ${Math.round((detection.yellowPercentage / 100) * detection.totalPixels).toLocaleString()}`);
        ...
        const croppedImage = await imageProcessor.cropImage(image, 80, 50);
        const croppedDetection = await imageProcessor.detectColors(croppedImage, threshold);
        ...
        // Comparación
        console.log('\n📈 COMPARACIÓN:');
        console.log(`   Rojo: ${detection.redPercentage}% → ${croppedDetection.redPercentage}% (${(croppedDetection.redPercentage - detection.redPercentage).toFixed(2)}%)`);
        console.log(`   Amarillo: ${detection.yellowPercentage}% → ${croppedDetection.yellowPercentage}% (${(croppedDetection.yellowPercentage - detection.yellowPercentage).toFixed(2)}%)`);

        // Guardar imagen recortada
        const croppedOutputPath = path.join(__dirname, '../public/images/test-detection-cropped.jpg');
        await imageProcessor.saveImage(croppedImage, croppedOutputPath);
