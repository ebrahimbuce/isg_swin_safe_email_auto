...
async function testAlertLevels() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      const detection = await imageProcessor.detectColors({ image: testImage, threshold: 0.5 });
      ...
      // 5. Validar
      const passed = alertStatus.level === testCase.expectedLevel && alertStatus.label === testCase.expectedLabel;
      ...
      await imageProcessor.saveImage({ image: testImage, path: outputPath });
